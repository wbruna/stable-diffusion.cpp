...
public:
    ...
    bool init_model_loader(ModelLoader& model_loader, ModelConfig& configuration) {
        const auto* sd_ctx_params_kcpp = &configuration.params;
...
static bool generate_image_impl(sd_ctx_t* sd_ctx,
...
SD_API bool generate_image(sd_ctx_t* sd_ctx,
    ...
    return execution.ready && generate_image_impl(sd_ctx, params, images_out, num_images_out);
}
...
static std::optional<ImageGenerationLatents> prepare_video_generation_latents(sd_ctx_t* sd_ctx,
    ...
    if (sd_version_is_ltxav(sd_ctx->sd->version)) {
        auto ltx_audio_vae_model = std::dynamic_pointer_cast<LTXV::LTXAudioVAERunner>(sd_ctx->sd->audio_vae_model);
        ...
        if (sd_vid_gen_params->input_audio != nullptr &&
            ...
            if (ltx_audio_vae_model == nullptr || !ltx_audio_vae_model->config.has_encoder) {
            ...
            auto waveform              = sd_audio_to_ltx_waveform_tensor(sd_vid_gen_params->input_audio,
                                                                         ltx_audio_vae_model->config.sample_rate,
                                                                         ltx_audio_vae_model->config.audio_channels);
            if (waveform.empty()) {
                LOG_ERROR("failed to convert source audio for LTX A2V encoding");
                return std::nullopt;
            }

            auto encoded_audio_latent = ltx_audio_vae_model->encode(sd_ctx->sd->n_threads, waveform);
...
static sd::Tensor<float> upscale_ltx_spatial_video_latent(sd_ctx_t* sd_ctx,
                                                          ...
                                                          int audio_length) {
    if (sd_ctx == nullptr || sd_ctx->sd == nullptr || sd_ctx->sd->model_manager == nullptr || packed_latent.empty()) {
        return {};
    }
    ...
    if (sd_ctx->sd->first_stage_model) {
        sd_ctx->sd->first_stage_model->runner_end();
    }
    ...
    auto model_manager = sd_ctx->sd->model_manager;
    ...
    upsampler                         = std::make_unique<LTXVUpsampler::LatentUpsamplerRunner>(sd_ctx->sd->backend_for(SDBackendModule::UPSCALER),
                                                                       model_manager->loader().get_tensor_storage_map(),
                                                                       prefix,
                                                                       model_manager);
    const size_t max_graph_vram_bytes = sd_ctx->sd->max_graph_vram_bytes_for_module(SDBackendModule::UPSCALER);
...
static bool generate_animatediff_video(sd_ctx_t* sd_ctx,
    ...
    bool ok                            = generate_image_impl(sd_ctx, &img_gen_params, frames_out, num_frames_out);
