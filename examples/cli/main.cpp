...
void parse_args(int argc, const char** argv, SDParams& params) {
    ...
    options.float_options = {
        ...
        {"", "--apg-eta", "", &params.sample_params.guidance.apg.eta},
        {"", "--apg-momentum", "", &params.sample_params.guidance.apg.momentum},
        {"", "--apg-nt", "", &params.sample_params.guidance.apg.norm_threshold},
        {"", "--apg-nt-smoothing", "", &params.sample_params.guidance.apg.norm_threshold_smoothing},
    ...
    options.bool_options = {
        ...
        {"", "--slg-uncond", "", true, &params.sample_params.guidance.slg.uncond},
...
std::string get_image_params(SDParams params, int64_t seed) {
    ...
    if (params.sample_params.guidance.apg.eta != 1) {
        parameter_string += "APG eta: " + std::to_string(params.sample_params.guidance.apg.eta) + ", ";
    }
    if (params.sample_params.guidance.apg.momentum != 0) {
        parameter_string += "CFG momentum: " + std::to_string(params.sample_params.guidance.apg.momentum) + ", ";
    }
    if (params.sample_params.guidance.apg.norm_threshold != 0) {
        parameter_string += "CFG normalization threshold: " + std::to_string(params.sample_params.guidance.apg.norm_threshold) + ", ";
        if (params.sample_params.guidance.apg.norm_threshold_smoothing != 0) {
            parameter_string += "CFG normalization threshold: " + std::to_string(params.sample_params.guidance.apg.norm_threshold_smoothing) + ", ";
        }
    }
    if (params.sample_params.guidance.slg.scale != 0 && params.skip_layers.size() != 0) {
        parameter_string += "Unconditional SLG: " + std::string(params.sample_params.guidance.slg.uncond ? "True" : "False") + ", ";
