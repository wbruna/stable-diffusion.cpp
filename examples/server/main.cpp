...
int main(int argc, const char** argv) {
    ...
    auto a1111_any2img = [&](const httplib::Request& req, httplib::Response& res, bool img2img) {
        try {
            if (req.body.empty()) {
                res.status = 400;
                res.set_content(R"({"error":"empty body"})", "application/json");
            ...
            // TODO these should probably be limited  by an estimated cost instead
            // (e.g. steps * image area * batch_size)
    ...
    svr.Post("/sdapi/v1/txt2img", [&](const httplib::Request& req, httplib::Response& res) {
        a1111_any2img(req, res, false);
    });

    svr.Post("/sdapi/v1/img2img", [&](const httplib::Request& req, httplib::Response& res) {
        a1111_any2img(req, res, true);
    });

    svr.Get("/sdapi/v1/samplers", [&](const httplib::Request&, httplib::Response& res) {
        // [{"name":"Euler","aliases":["k_euler"],"options":{}},{"name":"Euler a","aliases":["k_euler_a","k_euler_ancestral"],"options":{}},...]
        // for now, just include the name as the first alias
